...
pub fn save_project(
    ...
    let saved_data = fs::read_to_string(&file_path)
        .map_err(|e| format!("Failed to verify finalized project file: {}", e))?;
    if saved_data != project_data {
        return Err(
            "Project verification failed: finalized file differs from the saved payload"
                .to_string(),
        );
    }

    let bytes_written = fs::metadata(&file_path)
